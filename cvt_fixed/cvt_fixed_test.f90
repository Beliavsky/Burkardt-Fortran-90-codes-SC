...
!******************************************************************************
!
!! MAIN is the main program for CVT_FIXED_TEST.
!
!  Discussion:
!
!    CVT_FIXED_TEST tests the CVT_FIXED library.
!
...
!    cell centroids.
!
!    Local, integer ( kind = 4 ) CELL_FIXED(CELL_NUM), is 0 for "free" generators,
!    and 1 for "fixed" generators which cannot move.
!
!    Local, real ( kind = 8 ) CELL_GENERATOR(DIM_NUM,cell_num), the Voronoi
!    cell generators.
!
!    Local, integer ( kind = 4 ) CELL_NUM, the number of generators.
!
!    Local, integer ( kind = 4 ) CELL_SAMPLE, roughly the number of sampling
...
!    a crude control.
!
!    Local, integer ( kind = 4 ) CENTROID_IT, a counter for the CVT centroid
!    iteration.
!
!    Local, integer ( kind = 4 ) CENTROID_IT_MAX, the maximum number of steps
...
!
!    Local, integer ( kind = 4 ) DIM_NUM, the spatial dimension.
!
!    Local integer ( kind = 4 ) PPMB_NROW, PPMB_NCOL, the number of rows
...
!
!    Local, integer ( kind = 4 ) SAMPLE_NUM, the number of sample points, which
!    is the product of cell_num, the number of cells and
!    cell_sample, the number of sample points per cell.
!
!    Local, integer ( kind = 4 ) SEED, the seed for the random number generator.
