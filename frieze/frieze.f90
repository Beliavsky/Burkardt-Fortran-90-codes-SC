...
!  Parameters:
!
!    Output, integer ( kind = 4 ) IEND(MAX_END), contains the indices of XPAT
!    and YPAT that represent ends of line segments.  The first line segment 
!    involves the entries IEND(1) + 1 through IEND(2) of XPAT and YPAT, and the 
!    last one involves IEND(NUM_END-1) + 1 through IEND(NUM_END).
!
!    Input, integer ( kind = 4 ) MAX_END, the maximum number of entries in IEND.
!
!    Output, integer ( kind = 4 ) NUM_END, the number of entries used in IEND, 
!    and one more than the number of line segments used to draw the pattern.
!
!    Input, integer ( kind = 4 ) MAX_PAT, the maximum number of entries in 
!    XPAT and YPAT.
!
!    Output, integer ( kind = 4 ) NUM_PAT, the number of entries used in XPAT 
!    and YPAT.
!
