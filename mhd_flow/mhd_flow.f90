...
!
!    Local, integer ( kind = 4 ) ELEMENT_MAX, the maximum number of elements.
!
!    Local, integer ( kind = 4 ) ELEMENT_NUM, the number of elements.
!
!    Input, integer ( kind = 4 ) INDX(5,NODE_NUM), at each node, lists the
!    index of the finite element coefficients for U, V, B1, B2, and P.
!
!    Input, integer ( kind = 4 ) ELEMENT_NODE(6,ELEMENT_NUM), the nodes of 
!    each element.
!
!    Local, integer ( kind = 4 ) NODE_NUM, the number of nodes.
!
!    Local, integer ( kind = 4 ) NX, controls the number of elements in 
!    the X direction.
!
!    Local, integer ( kind = 4 ) NY, controls the number of elements in 
!    the Y direction.
!
...
!
!    Output, integer ( kind = 4 ) UNKNOWN_NUM, the number of unknowns.
!
...
!
!    Input, integer ( kind = 4 ) Y_IN_NODE, nodes 1 through Y_IN_NODE are on the
!    inflow boundary.
!
!    Input, real ( kind = 8 ) Y_INFLOW, the maximum Y coordinate at which
!    there is inflow.
!
!    Input, integer ( kind = 4 ) Y_OUT_NODE, nodes Y_OUT_NODE through NODE_NUM
...
!
  ...
  integer ( kind = 4 ), parameter :: nx = 9
  integer ( kind = 4 ), parameter :: ny = 9

  integer ( kind = 4 ), parameter :: element_max = 2 * ( nx - 1 ) * ( ny - 1 )
  ...
  integer ( kind = 4 ) :: y_in_node
  integer ( kind = 4 ) :: y_out_node
...
!
  ...
  write(*,*)  ' inflow boundary conditions:  u  quadratic from 0 to ', &
    y_inflow
  write(*,*)  ' outflow boundary conditions: grad u dot n=0  from ', &
    y_outflow, ' to ', ylength
...
!
  ...
  integer ( kind = 4 ) :: maxu     ! leading dimension of a
...
!
  ...
  integer ( kind = 4 ) i
  integer ( kind = 4 ), dimension(5,node_num) :: indx
  integer ( kind = 4 ) ip
  integer ( kind = 4 ) ipp
  integer ( kind = 4 ) iq
  integer ( kind = 4 ) iqq
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) NX, NY, the number of rows and columns 
!    of elements.
!
!    Input, integer ( kind = 4 ) ELEMENT_NUM, the number of elements.
!
!    Output, integer ( kind = 4 ) ELEMENT_NODE(6,ELEMENT_NUM), the nodes of 
!    each element.
!
  implicit none
...
!    index of the finite element coefficients for U, V, B1, B2, and P.
!
!    Input, integer ( kind = 4 ) ELEMENT, the index of the element.
!
!    Input, integer ( kind = 4 ) NODE_NUM, the number of nodes.
!
!    Input, integer ( kind = 4 ) ELEMENT_NODE(6,ELEMENT_NUM), the nodes of 
...
!    are on the outflow boundary.
!
!    Output, integer ( kind = 4 ) INDX(5,NODE_NUM), at each node, lists the
!    index of the finite element coefficients for U, V, B1, B2, and P.
!
!    Output, integer ( kind = 4 ) UNKNOWN_NUM, the number of unknowns.
!
...
!    index of the finite element coefficients for U, V, B1, B2, and P.
!
!    Input, integer ( kind = 4 ) ELEMENT_MAX, the maximum number of elements.
!
!    Input, integer ( kind = 4 ) NODE_NUM, the number of nodes.
!
!    Input, integer ( kind = 4 ) ELEMENT_NUM, the number of elements.
!
!    Input, integer ( kind = 4 ) N_LBAND, the half-bandwidth of the matrix.
!
!    Input, integer ( kind = 4 ) ELEMENT_NODE(6,ELEMENT_NUM), the nodes of 
!    each element.
!
...
!
!    Input, integer ( kind = 4 ) UNKNOWN_NUM, the number of unknowns.
!
...
!
  ...
  integer ( kind = 4 ), dimension(5,node_num) :: indx
  integer ( kind = 4 ) :: ip
  integer ( kind = 4 ) :: iuk_b1
  integer ( kind = 4 ) :: iuk_b2
  integer ( kind = 4 ) iunit
...
!
  ...
  integer ( kind = 4 ), dimension(5,node_num) :: indx
  integer ( kind = 4 ) :: iter
  integer ( kind = 4 ) :: max_newton
  integer ( kind = 4 ) :: n_lband
  integer ( kind = 4 ) :: n_simple
  real ( kind = 8 ) :: normf
  real ( kind = 8 ) :: par_sim
  integer ( kind = 4 ) :: quad_num
  real ( kind = 8 ) :: rey_mag
...
!    Input, integer ( kind = 4 ) ELEMENT_NUM, the number of elements.
!
!    Input, integer ( kind = 4 ) NODE_NODE, the number of nodes.
!
!    Input, integer ( kind = 4 ) ELEMENT_NODE(6,ELEMENT_NUM), the nodes of 
!    each element.
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ID, identifies the equation whose right 
!    hand side function is to be evaluated:
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ID, is -1 if the inflow function is to 
!    be evaluated, and any other value to evaluate the outflow.
!
...
!
  ...
  integer ( kind = 4 ), dimension(5,node_num) :: indx
  integer ( kind = 4 ) :: ip
  integer ( kind = 4 ) :: iuk_u
  integer ( kind = 4 ) :: iuk_v
  integer ( kind = 4 ) iunit
  real ( kind = 8 ) :: u_value
  real ( kind = 8 ) :: ubdry
...
!    Input, integer ( kind = 4 ) NODE_NUM, the number of nodes.
!
!    Input, integer ( kind = 4 ) NX, NY, the number of elements in the 
!    X and Y directions.
!
