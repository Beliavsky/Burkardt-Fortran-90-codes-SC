...
!*****************************************************************************80
!
!! MAIN is the main program for TABLE_SHIFT.
!
!  Discussion:
!
!    TABLE_SHIFT is the main routine of a program to "shift" a dataset.
!
!    There are points X(1:M,1:N) in the dataset.
!
!    The user is expected to specify a shift value S(1:M), and
...
!
  ...
  write ( *, '(a)' ) '  get a shift vector S(1:M),'
  write ( *, '(a)' ) '  write the shifted data X(1:M,1:N) + S(1:M) to a file.'
!
...
!
  ...
  call file_name_ext_swap ( output_filename, 'shift.txt' )
  ...
  call dtable_data_read ( input_filename, m, n, table )
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Enter the shift vector:'
  read ( *, * ) s(1:m)

  do i = 1, m
    table(i,1:n) = table(i,1:n) + s(i)
  end do

  call r8mat_print_some ( m, n, table, 1, 1, 5, 5, &
    '  5 by 5 portion of shifted data:' )

  call r8mat_write ( output_filename, m, n, table )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Wrote the shifted data to "' &
...
!*****************************************************************************80
!
!! DTABLE_DATA_READ reads double precision table data from a file.
!
!  Discussion:
!
...
!
!    Output, real ( kind = 8 ) ( kind = 8 ) TABLE(M,N), the table data.
!
  implicit none
...
!
!    Output, real ( kind = 8 ) ( kind = 8 ) RVEC(N), the values read from 
!    the string.
!
!    Output, integer ( kind = 4 ) IERROR, error flag.
!    0, no errors occurred.
