...
!
  ...
  if ( x < xmin ) then
    ...
    n = int ( y )
    f = y - real ( n, kind = 4 )
    n = int ( 23.0E+00 * xint + real ( n, kind = 4 ) )
    n16 = n / 16
    if ( n < 0 ) then
      n16 = n16 - 1
    end if
...
!    Input, real ( kind = 4 ) X, the argument.
!
!    Input, real ( kind = 4 ) ALGAP1, the logarithm of the gamma function 
!    of A+1.
!
...
!
  ...
  if ( nmin == 0 ) then
    ...
    nmin = int ( aln2b * real ( i4_mach ( 12 ), kind = 4 ) )
    nmax = int ( aln2b * real ( i4_mach ( 13 ), kind = 4 ) )
  end if
...
!
  ...
  seed = mod ( seed, ( i4_huge / 125 ) )
  seed = seed * 125
  seed = seed - ( seed / 2796203 ) * 2796203
  r4_ren = real ( seed, kind = 4 ) / 2796203.0E+00
...
!
  ...
  if ( niter == 0 ) then
    niter = int ( 1.443E+00 * r4_log ( - 0.104E+00 &
...
!
  ...
  real ( kind = 8 ), parameter :: pi2 = 1.57079632679489661923132169163975D+00
  real ( kind = 8 ) r8_acos
  real ( kind = 8 ) r8_asin
  real ( kind = 8 ) x
...
!    Input, real ( kind = 8 ) X, the argument.
!
!    Output, real ( kind = 8 ) R8_ERF, the error function at X.
!
  implicit none

  real ( kind = 8 ), parameter :: erfcs(21) = (/ &
    -0.49046121234691808039984544033376D-01, &
    -0.14226120510371364237824741899631D+00, &
    +0.10035582187599795575754676712933D-01, &
    -0.57687646997674847650827025509167D-03, &
    +0.27419931252196061034422160791471D-04, &
    -0.11043175507344507604135381295905D-05, &
    +0.38488755420345036949961311498174D-07, &
    -0.11808582533875466969631751801581D-08, &
  ...
  real ( kind = 8 ), parameter :: sqrtpi = &
    1.77245385090551602729816748334115D+00
  real ( kind = 8 ) value
  real ( kind = 8 ) x
...
!
  ...
  if ( x < xmin ) then
    ...
    n = int ( y )
    f = y - real ( n, kind = 8 )
    n = int ( 23.0D+00 * xint + real ( n, kind = 8 ) )
    n16 = n / 16
    if ( n < 0 ) then
      n16 = n16 - 1
    end if
...
!
  ...
  if ( eta < dos(nos) ) then
    ...
    write ( *, '(a)' ) '  ETA may be too small.'
    write ( *, '(a)' ) '  The requested accuracy cannot be guaranteed'
    write ( *, '(a)' ) '  even if all available coefficients are used.'
    value = nos
...
!
  ...
  if ( npart == 0 ) then
    ...
    npart = int ( log ( xmax ) / log ( scale ) + 1.0D+00 )
  end if
!
!  X may be too small.
!
...
!    Input, real ( kind = 8 ) X, the argument.
!
!    Input, real ( kind = 8 ) ALGAP1, the logarithm of the gamma 
!    function of A+1.
!
...
!
  ...
  if ( nmin == 0 ) then
    ...
    nmin = int ( aln2b * real ( i4_mach ( 15 ), kind = 8 ) )
    nmax = int ( aln2b * real ( i4_mach ( 16 ), kind = 8 ) )
  end if
...
!  Parameters:
!
!    Input, real ( kind = 8 ) R, determines the action.
!    * R = 0.0, the next random number of the sequence is generated.
!    * R < 0.0, the last generated number will be returned for
!    possible use in a restart procedure.
!    * R > 0.0, the sequence of random numbers will start with the
...
!    R4_RAND provided the arithmetic is done exactly.
!
!    Input/output, integer ( kind = 4 ) IX0, IX1, two parameters used
!    as seeds for the random number generator.  On first call, these
!    might both be set to 0.
!
!    Output, real ( kind = 8 ) R8_RAND, a pseudo-random number between 
!    0.0 and 1.0.
!
  implicit none

  integer ( kind = 4 ), parameter :: ia0 = 1029
  integer ( kind = 4 ), parameter :: ia1 = 1536
  integer ( kind = 4 ), parameter :: ia1ma0 = 507
  integer ( kind = 4 ), parameter :: ic = 1731
  integer ( kind = 4 ) ix0
  integer ( kind = 4 ) ix1
  integer ( kind = 4 ) iy0
  integer ( kind = 4 ) iy1
  ...
  if ( 0.0D+00 < r ) then
    ix1 = int ( mod ( r, 1.0D+00 ) * 4194304.0D+00 + 0.5D+00 )
    ix0 = mod ( ix1, 2048 )
    ix1 = ( ix1 - ix0 ) / 2048
  end if

  r8_rand = real ( ix1 * 2048 + ix0, kind = 8 )
  r8_rand = r8_rand / 4194304.0D+00
 
  return
end
function r8_randgs ( xmean, sd, seed )
...
!  Parameters:
!
!    Input, real ( kind = 8 ) XMEAN, the mean of the Gaussian distribution.
!
!    Input, real ( kind = 8 ) SD, the standard deviation of the 
!    Gaussian function.
!
!    Input/output, integer ( kind = 4 ) SEED, a seed for the random 
!    number generator.
!
!    Output, real ( kind = 8 ) R8_RANDGS, a normally distributed random number.
!
  implicit none
  ...
  r8_randgs = - 6.0D+00
  do i = 1, 12
    r8_randgs = r8_randgs + r8_ren ( seed )
  end do

  r8_randgs = xmean + sd * r8_randgs

  return
end
function r8_random ( n, t, ix0, ix1 )
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) N, the number of random numbers in an 
!    auxiliary table.
!
!    Input/output, real ( kind = 8 ) T(N+1), an array of random numbers,
!    initialized before first call by R8_RANDOM_INIT.
!
!    Input/output, integer ( kind = 4 ) IX0, IX1, two parameters used
!    as seeds for the random number generator.
!
!    Output, real ( kind = 8 ) R8_RANDOM, a random number between 0.0 and 1.0.
!
  implicit none
...
!
  j = int ( t(n+1) * real ( abs ( n ), kind = 8 ) + 1.0D+00 )
  t(n+1) = t(j)
  r8_random = t(j)
!
...
subroutine r8_random_init ( n, t, ix0, ix1 )
...
!    auxiliary table.
!
!    Output, real ( kind = 8 ) T(N+1), an array of random numbers. 
!
!    Input/output, integer ( kind = 4 ) IX0, IX1, two parameters used
...
!
  ...
  r8_ren = real ( seed, kind = 8 ) / 2796203.0D+00
...
!
  ...
  real ( kind = 8 ) f
  integer ( kind = 4 ) n2
  integer ( kind = 4 ), save :: ntsn = 0
  real ( kind = 8 ), parameter :: pi2rec = &
    0.63661977236758134307553505349006D+00
  real ( kind = 8 ), parameter :: pihi = 3.140625D+00
  real ( kind = 8 ), parameter :: pilo = 9.6765358979323846264338327950288D-04
  real ( kind = 8 ), parameter :: pirec = 0.31830988618379067153776752674503D+00
  integer ( kind = 4 ) r8_inits
  real ( kind = 8 ) r8_csevl
  real ( kind = 8 ) r8_mach
  real ( kind = 8 ) r8_sin
  ...
  real ( kind = 8 ), save :: xwarn = 0.0D+00
  real ( kind = 8 ) y
!
!  Initialize.
!
...
!
  ...
  if ( niter == 0 ) then
    niter = int ( 1.443D+00 * r8_log ( - 0.104D+00 &
