...
!
!    integer ( kind = 4 ) INDX(NP,2), records, for each node, whether
...
!    F and G arrays of the unknown coefficient.
!
!    integer ( kind = 4 ) INSC(NP), records, for each node, whether an
...
!    F and G arrays of the unknown coefficient.
!
!    integer ( kind = 4 ) IOUNIT, the FORTRAN unit number for the file
!    into which graphics data will be written.
!
!    integer ( kind = 4 ) IPIVOT(MAXEQN), pivot information used by the linear
!    solver.
!
!    integer ( kind = 4 ) ISOTRI(NELEMN), records whether or not a given
!    element is isometric or not.
!    0, element is not isometric.
!    1, element is isometric.
!
!    Input, integer ( kind = 4 ) IVUNIT, the output unit to which the data is to
!    be written.  The data file is unformated.
!
!    integer ( kind = 4 ) IWRITE, controls the amount of output produced by the
!    program.
!    0, minimal output.
...
!    2, copious output.
!
!    Input, integer ( kind = 4 ) IXUNIT, the output unit to which the data is to
!    be written.  The data file is unformated.
!
!    LOGICAL LONG,
...
!
!    integer ( kind = 4 ) MAXEQN, the maximum number of equations allowed.
!
!    integer ( kind = 4 ) MAXNEW, the maximum number of Newton steps 
!    per iteration.
!
!    integer ( kind = 4 ) MAXROW, the maximum row real ( kind = 8 ) of the 
!    coefficient matrix that is allowed.
!
!    integer ( kind = 4 ) MAXSEC, the maximum number of secant steps allowed.
!
!    integer ( kind = 4 ) MX, MX = 2*NX-1, the total number of grid points on
!    the horizontal side of the region.
!
!    integer ( kind = 4 ) MY, MY = 2*NY-1, the total number of grid points on
!    the vertical side of the region.
!
!    integer ( kind = 4 ) NELEMN, the number of elements used.
!
!    integer ( kind = 4 ) NEQN, the number of equations or functions for
!    the full system.
!
!    integer ( kind = 4 ) NLBAND, the number of diagonals below the main 
!    diagonal of the matrix A which are nonzero.
!
!    integer ( kind = 4 ) NNODES, the number of nodes per element, 6.
!
!    integer ( kind = 4 ) NODE(NELEMN,6), records the global node numbers of the
!    6 nodes that make up each element.
!
!    integer ( kind = 4 ) NODEX0, the lowest numbered node in the column of
!    nodes where the profile is measured.
!
!    integer ( kind = 4 ) NP, the number of nodes.
!
!    integer ( kind = 4 ) NQUAD, the number of quadrature points, currently
!    set to 3.
!
!    integer ( kind = 4 ) NROW, the used row dimension of the coefficient
!    matrix.
!
!    integer ( kind = 4 ) NUMNEW, total number of Newton iterations taken in
!    the NSTOKE routine during the entire run.
!
!    integer ( kind = 4 ) NUMSEC, the number of secant steps taken.
!
!    integer ( kind = 4 ) NX, the number of "main" grid points on the horizontal
!    side of the region.
!
!    integer ( kind = 4 ) NY, the number of "main" grid points on the vertical
!    side of the region.
!
...
!
  ...
  integer ( kind = 4 ) iter
  integer ( kind = 4 ) ivunit
  integer ( kind = 4 ) iwrite
  integer ( kind = 4 ) ixunit
  integer ( kind = 4 ) j
  logical long
  integer ( kind = 4 ) maxnew
  integer ( kind = 4 ) maxsec
...
!
  ...
  integer ( kind = 4 ) i,incx,incy,ix,iy,m,n
!
  if ( n <= 0)return
...
!                an integer ( kind = 4 ) vector of pivot indices.
!
!        info    integer ( kind = 4 )
!                = 0  normal value.
...
!    value, real or integer ( kind = 4 ), is written per record.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) NPARA, the number of parameters.  Fixed at 1
!    for now.
!
