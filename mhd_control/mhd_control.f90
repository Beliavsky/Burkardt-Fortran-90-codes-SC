...
!    off arrays.
!
!    BASIS_LINEAR   evaluates linear basis function centered at given node 
!    at (x,y)
!
!    BASIS_QUAD  evaluates basis function and x and y derivatives at given point
!
!    GEOMETRY  sets up the geometry for the problem giving x,y coordinates of 
!    each point, the node array and the index array
!
!    COST_FNL  evaluates the cost functional given the state and target
!
!    DECOMPOSE takes a 1-d array of unknowns indexed by unknown number and puts
!    into a 2-D array with row being node numnber and column u,v,B1 or B2
!
!    ERROR calculates the L^2 error  between state and target normalized by
!    L^2 error of target
!
!    EVAL  evaluates the velocity and magnetic field at a point (x,y) given 
!    the nodal values in a 1-d indexed by unknown number
!
!    EVAL_2d evaluates solution whose nodal values are given in terms of a 
!    2-d array
!
...
!    equations.  Uses a combination of Simple and Newton iteration
!
...
!  PARAMETERS
!
  real ( kind = 8 ), parameter :: coupling_par = 10.0D+00
  real ( kind = 8 ), parameter :: reynold = 1.0D+00
  real ( kind = 8 ), parameter :: rey_mag = 1.0D+00
  real ( kind = 8 ), parameter :: visc = 1.0D+00 / reynold

!
!  geometry
!
! number of points in x,y directions (excluding midpoints).
!
   integer ( kind = 4 ), parameter  :: nx =9 , ny =9 
! 
! max number of elements for dimensioning
!      
   integer ( kind = 4 ), parameter  :: max_elements =  2*( nx-1)* (ny-1)
!
! max number of nodes for dimensioning
!
   integer ( kind = 4 ), parameter  :: max_nodes =  (2*nx-1) * (2*ny - 1) 
!
! number of local  nodes per element
!  
   integer ( kind = 4 ), parameter  :: n_local = 6 
!
! number of quadrature points for assembly
!       
   integer ( kind = 4 ), parameter  :: n_quad = 3     
!
! number of unknowns (u,v,b1,b2,p)
!     
   integer ( kind = 4 ), parameter  :: nuk = 5             
!
! size of domain
!
  real ( kind = 8 ), parameter :: xlength = 1., ylength = 1.      
!
!  control  parameters
!
  ...
  real ( kind = 8 ) :: alpha1  =  10.0D+00        !  coefficient in cost functional of  velocity - target
  real ( kind = 8 ) :: alpha2  =  10.0D+00        !  coefficient in cost functional of   magnetic field -
  real ( kind = 8 ) :: beta1  =  0.01D+00        !  coefficient in cost functional of control (rhs) for v
  real ( kind = 8 ) :: beta2  =  0.01D+00        !  coefficient in cost functional of control (rhs) for m
  real ( kind = 8 ) :: lambda_max = 20.0D+00      !  maximum value for stepsize in gradient method
  real ( kind = 8 ) :: tol_grad = 1.0D-03    !  convergence tolerance for gradient method
!
!  time stepping parameters
  integer ( kind = 4 ), parameter  ::  steps_time = 10         ! total  number of time steps
  real ( kind = 8 ), parameter ::   dt = 0.005D+00               !  fixed time step
...
!
    f_prev(1:n_unknowns) = 0.0D+00   ! final condition for adjoint equation
    soln_adj(1:max_nodes, 1:4   )  = 0.0D+00
...
!
!   control_new(1:n_nodes,1:2) = 0.0D+00
!
    control_new(1:n_nodes,1:2) = ( 1.0D+00 - lambda * beta1 ) &
      * control_old(1:n_nodes,1:2)

    control_new(1:n_nodes,3:4) = ( 1.0D+00 - lambda * beta2 ) &
...
!  INPUT
!
!  flag for determining how much is written out
!
...
!
! unknown numbering array (ordering: u,v, p)
!      
...
!  Initialize.
!
  a(1:n_unknowns,1:n_band) = 0.0D+00
  f_adj(1:n_unknowns) = 0.0D+00
!
