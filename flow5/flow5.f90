...
!  Discussion:
!
!    FLOW5 solves a steady state incompressible flow problem using 
!    finite elements.
!
...
!
!    integer ( kind = 4 ) IERROR, an error flag.
...
!
!    integer ( kind = 4 ) IPIVOT(MAXEQN), pivoting space for the linear 
!    system solver.
!
!    integer ( kind = 4 ) ISOTRI(MAXELM), 0/1, the element IS NOT/IS 
!    isoparametric.
!
!    integer ( kind = 4 ) MAXELM, the maximum number of elements.
!
!    integer ( kind = 4 ) MAXEQN, the maximum number of equations.
!
!    integer ( kind = 4 ) MAXNEW, the maximum number of Newton iterations.
!
!    integer ( kind = 4 ) MAXNP, the maximum number of nodes.
!
!    integer ( kind = 4 ) MAXQUAD, the maximum number of quadrature points.
!
!    integer ( kind = 4 ) MAXROW, the first dimension of the matrix A.
!
!    integer ( kind = 4 ) NELEM, the number of elements.
!
!    integer ( kind = 4 ) NEQN, the number of finite element equations.
!
!    integer ( kind = 4 ) NLBAND, the lower bandwidth of the matrix A.
!
!    integer ( kind = 4 ) NODE(6,MAXELM), the nodes that make up each element.
...
!      1-6-3
!
!    integer ( kind = 4 ) NP, the number of nodes.  NP=(2*NX-1)*(2*NY-1).
!
!    integer ( kind = 4 ) NROW, the number of rows needed to store the matrix A.
!
!    integer ( kind = 4 ) NX, controls the spacing of nodes and elements in
...
!
!    integer ( kind = 4 ) NY, controls the spacing of nodes and elements in
...
!
  ...
  integer ( kind = 4 ), parameter :: maxquad = 3
!
...
!
  integer ( kind = 4 ), parameter :: maxrow = 29*maxny
  integer ( kind = 4 ), parameter :: maxelm = 2*(maxnx-1)*(maxny-1)
  integer ( kind = 4 ), parameter :: maxeqn = &
...
!
!    Input, integer ( kind = 4 ) IELEM, the index of the element.
!
!    Input, integer ( kind = 4 ) IQ, the index of the basis function.
!
!    Input, integer ( kind = 4 ) NELEM, the number of elements.
!
!    Input, integer ( kind = 4 ) NODE(6,MAXELM), the nodes that make up 
!    each element.
!
...
!
!    Output, integer ( kind = 4 ) IPIVOT(N), the pivot vector.
!
...
!    ML and MU must be nonnegative, and no greater than N-1.
!
!    Input, integer ( kind = 4 ) IPIVOT(N), the pivot vector from SGB_FA.
!
...
!
!    Input, integer ( kind = 4 ) INDX(3,MAXNP).  
!
!    INDX(I,J) contains, for each node J, the index of U, V and P 
!    at that node, or 0 or a negative value.
! 
...
!    Input, integer ( kind = 4 ) NELEM, the number of elements.
! 
!    Input, integer ( kind = 4 ) NEQN, the number of finite element equations.
! 
!    Input, integer ( kind = 4 ) NLBAND, the lower bandwidth of the matrix A.  
! 
!    Input, integer ( kind = 4 ) NODE(6,MAXELM), the nodes that make up 
!    each element.
! 
...
!    NP=(2*NX-1)*(2*NY-1).
! 
!    Input, integer ( kind = 4 ) NROW, the number of rows need to store 
!    the matrix A.
!
...
!
!    Input, real ( kind = 8 ) INV_NU, the value of the inverse dynamic 
!    viscosity.
!
  implicit none
...
!    associated with that node.
!
!    Workspace, integer ( kind = 4 ) IPIVOT(MAXEQN), pivot space needed by the
!    matrix factorization and solving routines.
!
!    Input, integer ( kind = 4 ) MAXNEW, the maximum number of Newton 
!    iterations.
!
!    Input, integer ( kind = 4 ) NELEM, the number of elements.
! 
...
!    Input, integer ( kind = 4 ) IELEM, the index of the element.
!
!    Input, integer ( kind = 4 ) IN, the index of the basis function.
...
!
!    Output, real ( kind = 8 ) XMAX, the largest absolute value of the entries
!    in the array.
!
!    Output, integer ( kind = 4 ) IXMAX, the index of the entry of largest 
!    absolute value.
!
  implicit none
...
!    associated with that node.
!
!    Input, integer ( kind = 4 ) MAXROW, the first dimension of the matrix A.
!
!    Input, integer ( kind = 4 ) NELEM, the number of elements.
! 
!    Output, integer ( kind = 4 ) NLBAND, the lower bandwidth of the matrix A.
! 
!    Input, integer ( kind = 4 ) NODE(6,MAXELM), the nodes that make up 
!    each element.
! 
!    Input, integer ( kind = 4 ) NP, the number of nodes.  NP=(2*NX-1)*(2*NY-1).
! 
!    Output, integer ( kind = 4 ) NROW, the number of rows needed to store 
!    the matrix A.
!
  implicit none
...
!
!    Input, real ( kind = 8 ) ETAQUAD(MAXQUAD), the "Eta" quadrature
!    coordinates.
!
!    Input, integer ( kind = 4 ) ISOTRI(MAXELM), 0/1, the element IS NOT/IS 
!    isoparametric.
!
!    Input, integer ( kind = 4 ) NELEM, the number of elements.
!
...
!    Input, real ( kind = 8 ) XC(MAXNP), the X coordinates of the nodes.
!
!    Input, real ( kind = 8 ) XQUAD(MAXQUAD,NELEM), the X quadrature
!    coordinates.
!
!    Input, real ( kind = 8 ) XSIQUAD(MAXQUAD), the "Xsi" 
!    quadrature coordinates.
!
!    Input, real ( kind = 8 ) YC(MAXNP), the Y coordinates of the nodes.
!
!    Input, real ( kind = 8 ) YQUAD(MAXQUAD,MAXELM), the Y quadrature 
!    coordinates.
!
  implicit none
...
!    Input, real ( kind = 8 ) XC(MAXNP), the X coordinates of the nodes.
!
!    Output, real ( kind = 8 ) XQUAD(MAXQUAD,MAXELM), the X quadrature 
!    coordinates.
!
!    Output, real ( kind = 8 ) XSIQUAD(MAXQUAD), the "Xsi" quadrature 
!    coordinates.
!
!    Input, real ( kind = 8 ) YC(MAXNP), the Y coordinates of the nodes.
!
!    Output, real ( kind = 8 ) YQUAD(MAXQUAD,MAXELM), the Y quadrature 
!    coordinates. 
!
  implicit none
...
!    'PB' The condition P=0 applied at (XMAX,YMAX).
!
!    Output, integer ( kind = 4 ) INDX(3,MAXNP).  
!
!    INDX(I,J) contains, for each node J, the index of U, V and P 
!    at that node, or 0 or a negative value.
! 
...
!    associated with that node.
!
!    Output, integer ( kind = 4 ) ISOTRI(MAXELM), 0/1, the element IS NOT/IS 
!    isoparametric.
! 
!    Input, integer ( kind = 4 ) MAXEQN, the maximum number of equations.
!
!    Input, integer ( kind = 4 ) NELEM, the number of elements.
!
!    Output, integer ( kind = 4 ) NEQN, the number of finite element equations.
!
!    Input, integer ( kind = 4 ) NODE(6,MAXELM), the nodes that make up 
!    each element.
!
...
!    Input, integer ( kind = 4 ) NP, the number of nodes.
!
!    Input, integer ( kind = 4 ) NX.  NX controls the spacing of nodes and 
!    elements in the X direction.  There are 2*NX-1 nodes along various
!    lines in the X direction.  Roughly speaking, NX (or 2*NX) is the number 
!    of elements along a line in the X direction.
! 
!    Input, integer ( kind = 4 ) NY.  NY controls the spacing of nodes and 
!    elements in the Y direction.  There are 2*NY-1 nodes along various
...
!    associated with that node.
!
!    Input, integer ( kind = 4 ) IQUAD, the index of the quadrature point.
!
!    Input, integer ( kind = 4 ) NELEM, the number of elements.
! 
!    Input, integer ( kind = 4 ) NEQN, the number of finite element equations.
...
subroutine write_tecplot_file ( maxelm, maxnp, nelem, node, np, p, u, v, &
  xc, yc )
