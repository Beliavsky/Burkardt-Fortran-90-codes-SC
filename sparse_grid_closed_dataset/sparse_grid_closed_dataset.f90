...
!  Usage:
!
!    sparse_grid_closed_dataset dim_num level rule
!
!    where
!
!    * dim_num is the spatial dimension of the quadrature region,
!    * level is the level that defines the Smolyak grid.
!    * rule is
!      * "CC" for Clenshaw-Curtis
!      * "NCC" for Newton-Cotes Closed.
!
!  Licensing:
!
...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'SPARSE_GRID_CLOSED_DATASET'
  ...
  write ( *, '(a)' ) '  construction based on a closed 1D quadrature rule.'
  write ( *, '(a)' ) ' '
  ...
  write ( *, '(a)' ) '    "CC":  Clenshaw-Curtis;'
  write ( *, '(a)' ) '    "NCC": Newton-Cotes Closed.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Output from the program includes:'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '    A printed table of the abscissas and weights.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '    Files of the abscissas, weights, ranges.'
!
...
!
  ...
  if ( s_eqi ( rule_string, 'CC' ) ) then
    rule = 1
  else if ( s_eqi ( rule_string, 'NCC' ) ) then
    rule = 2
  else
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'SPARSE_GRID_CLOSED_DATASET - Fatal error!'
    write ( *, '(a)' ) '  Choice of rule not recognized.'
    stop
  end if
...
!
  call spgrid_closed_index ( dim_num, level_max, point_num, grid_index )

  call i4mat_transpose_print_some ( dim_num, point_num, grid_index, &
    1, 1, dim_num, 10, '  First 10 entries in grid index:' )
!
!  Compute the physical coordinates of the abscissas.
!  At this point, we can choose between the Clenshaw Curtis rule
!  or the Newton Cotes Closed rule, or even any mixed product of
!  these rules.
!
  if ( 0 == level_max ) then
    order_max = 1
  ...
  if ( rule == 1 ) then
    ...
    do point = 1, point_num
      do dim = 1, dim_num
        grid_point(dim,point) = &
          ncc_abscissa ( order_max, grid_index(dim,point) + 1 )
      end do
    end do
...
!  Gather the weights.
!
  call spgrid_closed_weights ( dim_num, level_max, point_num, grid_index, &
    rule, grid_weight )
...
!    Input, integer ( kind = 4 ) DIM_NUM, the spatial dimension.
!
!    Input, integer ( kind = 4 ) T(DIM_NUM), the grid indices of a point in
!    a 1D closed rule.  0 <= T(I) <= ORDER.
!
...
!
!    Input/output, integer ( kind = 4 ) GRID_INDEX(DIM_NUM,POINT_NUM), the
!    index values for each grid point.  On input, these indices are based in
...
!    Input, integer ( kind = 4 ) ORDER, the order of the rule.
!
!    Input, integer ( kind = 4 ) I, the index of the abscissa, between 1
!    and ORDER.
!
!    Output, real ( kind = 8 ) NCC_ABSCISSA, the location of the abscissa,
!    between -1.0 and +1.0.
!
  implicit none
  ...
  real    ( kind = 8 ), parameter :: x_min = -1.0D+00
  ...
  if ( order == 1 ) then
    ...
    value = ( x_max + x_min ) / 2.0D+00

  else

    value = ( real ( order - i,     kind = 8 ) * x_min   &
...
subroutine ncc_weights ( order, weight )
...
!  Discussion:
!
!    The integration interval is assumed to be [-1,1].
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
subroutine product_weights_closed ( dim_num, order_1d, order_nd, rule, w_nd )
...
!
  ...
  do dim = 1, dim_num
    ...
    if ( rule == 1 ) then
      ...
      call ncc_weights ( order_1d(dim), w_1d )
    end if
...
subroutine spgrid_closed_index ( dim_num, level_max, point_num, grid_index )
...
!    calling SPGRID_CLOSED_SIZE first.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
subroutine spgrid_closed_weights ( dim_num, level_max, point_num, grid_index, &
  rule, grid_weight )
...
!
      call product_weights_closed ( dim_num, order_1d, order_nd, rule, &
        grid_weight2 )
!
