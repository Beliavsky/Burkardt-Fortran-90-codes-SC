...
!    The output of this call is FILE_UNIT, a value which must be 
...
!    Input, character ( len = * ) FILE_NAME, the name of the file.
!
!    Input/output, integer ( kind = 4 ) FILE_UNIT, the unit number associated
!    with the file.  For the 'c' command, this value is output.
!    For the 'w', 'r', 'f' and 'd' commands, this value is input.
!    
!    Input, integer ( kind = 4 ) RECORD, the index of the record to be written
!    or read.  (This value is needed for the 'W' and 'R' actions only.)
!
!    Input, integer ( kind = 4 ) N, the size of each vector.
...
!
  ...
  integer ( kind = 4 ), parameter :: char_length = 16
  logical file_exist
  logical file_is_open
  character ( len = * ) file_name
