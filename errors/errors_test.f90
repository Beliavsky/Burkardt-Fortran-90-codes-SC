...
!*****************************************************************************80
!
!! MAIN is the main program for ERRORS_TEST.
!
!  Discussion:
!
!    ERRORS_TEST tests the ERRORS library.
!
...
!
  ...
  drm_bot = ( dx1 ** 2 + dx2 ** 2 + dx3 ** 2 - ( dx1 + dx2 + dx3 ) ** 2 &
    / 3.0D+00 )
...
!
  ...
  do i = 1, n
    do j = 1, n
      a(i,j) = real ( mult, kind = 4 ) / real ( i + j - 1, kind = 4 )
    end do
  end do
...
!
  ...
  do
    ...
    fx3 = 0.5E+00 * x ** 2 * ( 1.0E+00 - ( x ** 2 / 12.0E+00 ) &
      + ( x ** 4 / 360.0E+00 ) )
...
!
  ...
  write ( *, '(a)' ) '+=============================================+'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '    N  Formula 1  Formula 2  x^n/(n)'
  write ( *, '(a)' ) ' '
...
!
  ...
  write ( *, '(a)' ) '|  Exercise 27                                |'
  write ( *, '(a)' ) '|                                             |'
  write ( *, '(a)' ) '|  TAN(X) and ATAN(X) are inverse functions.  |'
  write ( *, '(a)' ) '|  Therefore, it should always be that:       |'
  write ( *, '(a)' ) '|                                             |'
  write ( *, '(a)' ) '|    TAN(ATAN(X)) = X                         |'
  write ( *, '(a)' ) '|                                             |'
  write ( *, '(a)' ) '+=============================================+'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '       X         TAN(ATAN(X))    Error'
  write ( *, '(a)' ) ''
  ...
  do i = 0, 20
    ...
    tatx = tan ( atx )
    e = abs ( tatx - x )
    write ( *, '(2x,g14.6,2x,g14.6,2x,g14.6)' ) x, tatx, e
    x = x * 10.0D+00
  end do
