...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) &
    '  Read the header of "' // trim ( input_file_name ) // '".'
  write ( *, '(a)' ) ' '
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) N, the number of positions to be filled 
!    in the vector.
!
!    Input/output, integer ( kind = 4 ) X(N), the partial or complete 
!    candidate vector.
!
...
!
!    Input, integer ( kind = 4 ) NOPEN, identifies the first open position 
!    in ORDER.
!
!    Input, integer ( kind = 4 ) ORDER(NNODE), contains in entries 1 
!    through NOPEN-1 the elements of the current partial list.
!
!    Input/output, integer ( kind = 4 ) STACK(MAXSTACK), used to store the 
!    new candidates.
!
...
!
!    Input, integer ( kind = 4 ) ORDER(NNODE), the ordering of the nodes.  
!    ORDER(1) is the first node, and so on.
!
  implicit none
