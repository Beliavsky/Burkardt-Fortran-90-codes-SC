...
!*****************************************************************************80
!
!! MAIN is the main program for TABLE_UNIFORM_NOISE.
!
!  Discussion:
!
!    TABLE_UNIFORM_NOISE is the main routine to add noise to a dataset.
!
!    There are points X(1:M,1:N) in the dataset.
!
!    The user is expected to specify a SEED for the random number 
!    generator, and a scale vector S(1:M).
!
!    The program writes a new dataset containing the N points
!
!      X(1:M,1:N) + S(1:M) * U(1:M,1:N)
!
!    where U(1:M,1:N) is a set of uniform random values.
!
!  Licensing:
!
...
!  Usage:
!
!    table_uniform_noise input_filename
!
  implicit none
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TABLE_UNIFORM_NOISE'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Read a dataset of N points in M dimensions,'
  write ( *, '(a)' ) '  get a SEED for the random number generator,'
  write ( *, '(a)' ) '  and a scale vector S(1:M).'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Add a uniformly random scaled shift to each value.'
  write ( *, '(a)' ) '    X(1:M,1:N) <= X(1:M,1:N) + S(1:M) * U(1:M,1:N).'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Write the noisy data to a file.'
!
...
!
  ...
  call file_name_ext_swap ( output_filename, 'uniform_noise.txt' )
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Enter the scale vector S:'
  write ( *, '(a)' ) '  (This limits the maximum value of the noise'
  write ( *, '(a)' ) '  in each component of the data.)'

  read ( *, * ) s(1:m)

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Enter the random number seed SEED:'
  write ( *, '(a)' ) '  (Use 123456789 if you have no preference.)'
  read ( *, * ) seed

  seed_save = seed
!
!  Add or subtract noise from each entry.
!
  do i = 1, m
    do j = 1, n
      table(i,j) = table(i,j) &
        + s(i) * ( 2.0D+00 * r8_uniform_01 ( seed ) - 1.0D+00 )
    end do
  end do
!
...
!
  call r8mat_print_some ( m, n, table, 1, 1, 5, 5, &
    '  5 by 5 portion of noisy data:' )
!
!  Write the table to a file.
!
  call r8mat_write ( input_filename, m, n, table )

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Wrote the noisy data to "' &
