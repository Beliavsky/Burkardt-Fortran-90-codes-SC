...
!
!    Local, integer ( kind = 4 ) IT_MAX, the maximum number of Newton 
!    iterations allowed.  0 is a legal value.  It simply solves the Stokes 
!    problem, computes the Navier-Stokes residual, and stops.
...
!
!    Local, integer ( kind = 4 ) NODE_TYPE(NODE_NUM), determines if the node 
...
!
!    Local, integer ( kind = 4 ) NODE3_NUM, the number of pressure nodes.
!
...
!
  ...
  integer ( kind = 4 ) ierr
  integer ( kind = 4 ) ip
  integer ( kind = 4 ), parameter :: it_max = 5
...
!
  ...
  do
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a,g14.6)' ) &
      '  l2-norm of adjusted FEM residual = ', node_r_norm
...
!    horizontal velocity variable associated with a node, or -1 if there 
!    is none.
!
...
!
!    Input, integer ( kind = 4 ) IB, the bandwidth of the jacobian.
!
!    Output, real ( kind = 8 ) A(3*IB+1,VARIABLE_NUM), the VARIABLE_NUM
...
!
!    Output, integer ( kind = 4 ) IVAL, the integer ( kind = 4 ) value read.
...
!    each element, the indices of the nodes that form the vertices.
!
