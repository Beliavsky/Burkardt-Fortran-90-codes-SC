...
!  Discussion:
!
!    PITCON66_PRB1 tests the PITCON library.
!
!    This example treats a system based on the Freudenstein-Roth function.
!
...
!
  xr(1:3) =  (/ 15.0D+00, -2.0D+00, 0.0D+00 /)
...
!
  ...
  f(1) = x(1) - ( ( x(2) - 5.0D+00 ) * x(2) + 2.0D+00 ) * x(2) - 13.0D+00 &
         + 34.0D+00 * ( x(3) - 1.0D+00 )

  f(2) = x(1) + ( ( x(2) + 1.0D+00 ) * x(2) - 14.0D+00 ) * x(2) - 29.0D+00 &
         + 10.0D+00 * ( x(3) - 1.0D+00 )
...
!
  ...
  fjac(1,1) = 1.0D+00
  fjac(1,2) = ( - 3.0D+00 * x(2) + 10.0D+00 ) * x(2) - 2.0D+00
  fjac(1,3) = 34.0D+00

  fjac(2,1) = 1.0D+00
  fjac(2,2) = ( 3.0D+00 * x(2) + 2.0D+00 ) * x(2) - 14.0D+00
  fjac(2,3) = 10.0D+00
