...
!
  ...
  character ( len = 255 ) dir_file_name
  integer ( kind = 4 ) i
  integer ( kind = 4 ) iarg
  ...
  character ( len = 255 ) vec_file_name
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) UNIT, the FORTRAN unit to which output was
!    written.
!
  implicit none
...
!
!    Output, integer ( kind = 4 ) THIN_NUM, the number of vectors remaining 
!    after thinning.
!
!    Output, integer ( kind = 4 ) THIN_DEX(NODE_NUM), contains in (1:THIN_NUM)
!    the indices into X and Y of the vectors to be retained after thinning.
!
  implicit none
