...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'INT_EXACTNESS_LAGUERRE'
...
!
  if ( 2 <= arg_num ) then
    ...
    write ( *, '(a)' ) 'INT_EXACTNESS_LAGUERRE:'
    write ( *, '(a)' ) &
      '  Enter DEGREE_MAX, the maximum monomial degree to check.'
...
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'INT_EXACTNESS_LAGUERRE: User input:'
...
!
  ...
  if ( dim_num /= 1 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'INT_EXACTNESS_LAGUERRE - Fatal error!'
...
!
  ...
  write ( *, '(a)'       ) '  a Gauss-Laguerre rule'
  write ( *, '(a,i8)'    ) '  of ORDER = ', order
  write ( *, '(a,g14.6)' ) '  with A =   ', a
  write ( *, '(a)'       ) ' '
  if ( option == 0 ) then
    write ( *, '(a)'       ) '  OPTION = 0, standard rule:'
    write ( *, '(a)'       ) '    Integral ( A <= x < oo ) exp(-x) f(x) dx'
...
!
  ...
  do degree = 0, degree_max
    ...
    call monomial_quadrature_laguerre ( degree, order, option, w, x, &
    quad_error )
...
subroutine monomial_quadrature_laguerre ( expon, order, option, w, x, &
  quad_error )
