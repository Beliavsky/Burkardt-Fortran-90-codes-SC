...
!    X and Y directions.
!
!    Output, real ( kind = 4 ) Q(IDIM,JDIM,4), the Q data, density, X and Y 
...
subroutine r4_b_2d_qfile ( iunit, idim, jdim, alpha, fsmach, re, time, q, &
  ierror )
...
!    Output, real ( kind = 4 ) TIME, the time associated with the data.
!
!    Output, real ( kind = 4 ) Q(IDIM,JDIM,4), the Q data, density, X and
...
!    X and Y directions.
!
!    Output, real ( kind = 4 ) X(IDIM,JDIM), Y(IDIM,JDIM), the X and Y 
...
!    X, Y, and Z directions.
!
!    Output, real ( kind = 4 ) Q(IDIM,JDIM,KDIM,5), the Q data, density, X, Y 
!    and Z momentum, and stagnation energy per unit volume.
!
!    Output, integer ( kind = 4 ) IERROR, error flag.
!    0, no error detected.
!    nonzero, a read error occurred.
...
!*****************************************************************************80
!
!! R4_B_3DM_Q reads a binary 3D multiple grid Q file for parameters and Q data.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
subroutine r4_b_3dm_qfile ( iunit, idim, jdim, kdim, maxi, maxj, maxk, &
...
!    the number of nodes in the X, Y, and Z directions for each grid.
!
!    Input, integer ( kind = 4 ) MAXI, MAXJ, MAXK, the maximum dimension for the
!    first three components of X, Y and Z.
!
!    Input, integer ( kind = 4 ) MAXGRID, the maximum value of NGRID.
!
!    Output, integer ( kind = 4 ) NGRID, the number of grids.
...
subroutine r8_b_2d_qfile ( iunit, idim, jdim, alpha, fsmach, re, time, q, &
  ierror )
...
!    X and Y directions.
!
!    Output, real ( kind = 8 ) X(IDIM,JDIM), Y(IDIM,JDIM), the X and Y 
!    coordinates of the nodes.
!
!    Output, integer ( kind = 8 ) IERROR, error flag.
!    0, no error detected.
...
!    X, Y, and Z directions.
!
!    Output, real ( kind = 8 ) Q(IDIM,JDIM,KDIM,5), the Q data, density, X, Y 
!    and Z momentum, and stagnation energy per unit volume.
!
!    Output, integer ( kind = 8 ) IERROR, error flag.
!    0, no error detected.
...
!*****************************************************************************80
!
!! R8_B_3DM_Q reads a binary 3D multiple grid Q file for parameters and Q data.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!    the number of nodes in the X, Y, and Z directions in each grid.
!
!    Input, integer ( kind = 8 ) MAXI, MAXJ, MAXK, the maximum dimension for 
!    the first three components of Q.
!
!    Input, integer ( kind = 8 ) MAXGRID, the maximum value of NGRID.
!
!    Input, integer ( kind = 8 ) NGRID, the number of grids.
!
!    Output, real ( kind = 8 ) FSMACH(MAXGRID), ALPHA(MAXGRID), RE(MAXGRID), 
!    TIME(MAXGRID), the values of the free stream Mach number, the angle of 
!    attack, in degrees, the Reynolds number for the flow, and the time, for
!    each grid.
!
...
!    for each grid.
!
!    Output, integer ( kind = 8 ) B(MAXI,MAXJ,MAXK,MAXGRID), the blanking 
!    information.
!
!    Output, integer ( kind = 8 ) IERROR, error flag.
!    0, no error detected.
...
subroutine r8_b_3dm_qfile ( iunit, idim, jdim, kdim, maxi, maxj, maxk, &
  maxgrid, ngrid, fsmach, alpha, re, time, q, ierror )
