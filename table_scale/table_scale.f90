...
!*****************************************************************************80
!
!! MAIN is the main program for TABLE_SCALE.
!
!  Discussion:
!
!    TABLE_SCALE is the main routine of a program to scale a dataset.
!
!    There are points X(1:M,1:N) in the dataset.
!
!    The user is expected to specify a scale vector S(1:M), and
...
!
  ...
  write ( *, '(a)' ) '  get a scale vector S(1:M),'
  write ( *, '(a)' ) '  write the scaled data S(1:M) * X(1:M,1:N) to a file.'
!
...
!
  ...
  call file_name_ext_swap ( output_filename, 'scale.txt' )
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Enter the scale vector:'
  read ( *, * ) s(1:m)

  do i = 1, m
    table(i,1:n) = s(i) * table(i,1:n)
  end do
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Wrote the scaled data to "' &
