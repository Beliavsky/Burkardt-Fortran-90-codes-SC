...
!  Parameters:
!
!    Input, integer ( kind = 4 ) PERM_INV(N), the inverse permutation, 
!    constructed by PERM_INVERSE.
!
!    Input, integer ( kind = 4 ) ISUB, the index of the entry of RHS to 
!    be modified.
!
...
!    has been permuted.
!
!    Input, real ( kind = 8 ) VALUE, the quantity to be added to the right 
!    hand side.
!
  implicit none
...
!    For each row, the column indices of the nonzero entries.
!
!    Input/output, integer ( kind = 4 ) ADJ(ADJ_MAX), stores adjacency 
!    information.
!
!    Input, integer ( kind = 4 ) IROW, JCOL, the row and column indices of
!    a nonzero entry of the matrix.
!
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, integer ( kind = 4 ) ADJ_ROW(N+1).  Information about row 
...
!    For each row, the column indices of the nonzero entries.
!
!    Input/output, integer ( kind = 4 ) PERM(N), the permutation vector, which
...
!  Local parameters:
!
!    Local, integer ( kind = 4 ) BNUM(N) stores the block number of 
!    each variable.
!
...
!
!    Output, integer ( kind = 4 ) DEG(N), the degrees of the nodes in 
!    the component.
!
!    Output, integer ( kind = 4 ) CCSIZE, the size of the component specified by
!    MASK and ROOT.
!
!    Output, integer ( kind = 4 ) LS(N), stores the nodes of the component 
!    level by level.
!
!    Input, integer ( kind = 4 ) N, the number of equations.
!
...
!
!    Workspace, integer ( kind = 4 ) MASK(N), marks nodes that have 
!    been considered.
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ROOT, the node that determines the 
!    masked component.
!
!    Input, integer ( kind = 4 ) ADJ_ROW(N+1).  Information about row I is
!    stored in entries ADJ_ROW(I) through ADJ_ROW(I+1)-1 of ADJ.
...
!    For each row, the column indices of the nonzero entries.
!
!    Output, integer ( kind = 4 ) NSEP, the number of variables in
!    the separator.
!
!    Output, integer ( kind = 4 ) SEP(*), the separator nodes.
!
!    Input/output, integer ( kind = 4 ) MASK(N), nodes in the separator have
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input/output, integer ( kind = 4 ) ADJ_ROW(N+1).  Information about row I
!    is stored in entries ADJ_ROW(I) through ADJ_ROW(I+1)-1 of ADJ.
!
!    Input, integer ( kind = 4 ) ADJ(*), the adjacency structure.
!    For each row, the column indices of the nonzero entries.
!
!    Input, integer ( kind = 4 ) PERM(N), PERM_INV(N), the permutation and 
!    inverse permutation vectors.
!
!    Output, integer ( kind = 4 ) XENV(N+1), the index vector for the level 
!    structure to be used to store the lower (or upper) envelope of the 
!    reordered matrix.
!
...
!  Parameters:
!
!    Input/output, integer ( kind = 4 ) ROOT.  On input, with the array NODLVL,
!    ROOT specifies the component whose pseudo-peripheral node is to be found.
...
!
!    Input/output, integer ( kind = 4 ) NODLVL(N).  On input, it specifies a 
!    section subgraph.  On return, the node level numbers.
!
  implicit none
...
!
!    Output, integer ( kind = 4 ) XNONZ(N+1), NZSUBS(*), the column subscripts
!    of the nonzeros of the matrix to the left of the diagonal blocks are
...
!
!    Input/output, integer ( kind = 4 ) NSPAN, SET(*), the input set.  On 
!    return, the resulting span set.
!
!    Input/output, integer ( kind = 4 ) NODLVL(N), the level number vector.  
!    Nodes considered will have their NODLVL changed to zero.
!
!    Output, integer ( kind = 4 ) NADJS, ADJS(*), the adjacency set of the span
!    in the lower level.
!
!    Output, integer ( kind = 4 ) LEAF.  If the span has an unnumbered higher 
!    level node, LEAF returns an unnumbered leaf node in the level
...
!    Input, integer ( kind = 4 ) NBLKS, XBLK(NBLKS+1), the tree partitioning.
!
!    Output, integer ( kind = 4 ) FATHER(N), the father vector of the quotient
!    tree.
!
!    Input, integer ( kind = 4 ) N, the number of equations.
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ROOT, the node that defines (along with MASK)
...
!    For each row, the column indices of the nonzero entries.
!
!    Output, integer ( kind = 4 ) NSEP, the number of nodes in the one-way
!    dissectors.
!
!    Output, integer ( kind = 4 ) SEP(*), the dissector nodes.
!
!    Input/output, integer ( kind = 4 ) MASK(N), nodes in the dissector have
!    their mask values set to zero.
!
!    Workspace, integer ( kind = 4 ) XLS(N+1), LS(N), the level structure used
!    by the routine ROOT_FIND.
!
!    Input, integer ( kind = 4 ) N, the number of equations.
!
  implicit none
...
!
!    Workspace, integer ( kind = 4 ) MARKER(N), a marker vector, where MARKER(I)
!    is negative means node I has been merged with another node and thus
!    can be ignored.
!
!    Workspace, integer ( kind = 4 ) RCHSET(*), used for the reachable set.
!
!    Workspace, integer ( kind = 4 ) NBRHD(*), a vector used for the 
!    neighborhood set.
!
!    Workspace, integer ( kind = 4 ) QSIZE(N), the sizes of indistinguishable
!    supernodes.
!
!    Workspace, integer ( kind = 4 ) QLINK(N), vector to store indistinguishable
!    nodes, I, QLINK(I), QLINK(QLINK(I)) are the members of the supernode
!    represented by I.
!
  implicit none
...
!  Local Parameters:
!
!    Local, integer ( kind = 4 ) XLS(N+1), the index vector for a level 
!    structure.  The level structure is stored in the currently unused spaces
!    in the permutation vector PERM.
!
!    Local, integer ( kind = 4 ) MASK(N), marks variables that have 
!    been numbered.
!
  implicit none
...
!    For each row, the column indices of the nonzero entries.
!
!    Output, integer ( kind = 4 ) NBLKS, XBLK(*), the quotient tree
!    partitioning.
!
...
!    by LEVEL_SET.
!
!    Local, integer ( kind = 4 ) MASK(N), marks variables that have been 
!    numbered.
!
  implicit none
...
!
!    Input, integer ( kind = 4 ) XNZSUB(*), NZSUB(*), the compressed subscript
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, integer ( kind = 4 ) IXLNZ(N+1), real ( kind = 8 ) LNZ(*), the 
!    structure of nonzeros in L.
!
!    Input, integer ( kind = 4 ) XNZSUB(*), NZSUB(*), the compressed subscript
!    structure.
!
...
!    For each row, the column indices of the nonzero entries.
!
!    Input/output, integer ( kind = 4 ) MASK(N).  On input, only nodes with 
!    nonzero MASK are to be processed.  On output, those nodes which were
...
!
!    Input, integer ( kind = 4 ) NHDSZE, NBHRD(*), the set of eliminated
!    supernodes adjacent to some nodes in the set.
!
!    Input/output, integer ( kind = 4 ) DEG(N), the degree vector.
!
!    Output, integer ( kind = 4 ) QSIZE(N), the size of indistinguishable nodes.
!
!    Output, integer ( kind = 4 ) QLINK(N), the linked list for 
...
!    For each row, the column indices of the nonzero entries.
!
!    Input, integer ( kind = 4 ) RCHSZE, RCHSET(*), the reachable set of root 
!    in the old quotient graph.
!
!    Input, integer ( kind = 4 ) NBRHD(*), the neighborhood set which will be 
!    merged with root to form the new supernode.
!
!    Input, integer ( kind = 4 ) MARKER(N), the marker vector.
!
!    Input/output, integer ( kind = 4 ) ADJ(*), becomes the adjacency
!    structure of the quotient graph.
!
!    Input, integer ( kind = 4 ) N, the number of equations.
!
  implicit none
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ROOT, the given node, which is not in 
!    the subset.
!
!    Input, integer ( kind = 4 ) ADJ_ROW(N+1).  Information about row I is 
!    stored in entries ADJ_ROW(I) through ADJ_ROW(I+1)-1 of ADJ.
...
!    For each row, the column indices of the nonzero entries.
!
!    Input, integer ( kind = 4 ) DEG(N), the degree vector.  DEG(I) < 0 means 
!    node I belongs to the given subset.
!
!    Output, integer ( kind = 4 ) RCHSZE, RCHSET(*), the reachable set.
!
!    Output, integer ( kind = 4 ) NHDSZE, NBRHD(*), the neighborhood set.
!
!    Input/output, integer ( kind = 4 ) MARKER(N), the marker vector for the 
!    reachable and neighborhood sets.
...
!    For each row, the column indices of the nonzero entries.
!
!    Input, integer ( kind = 4 ) NLIST, LIST(*), the nodes whose degree has
!    to be updated.
!
!    Input/output, integer ( kind = 4 ) DEG(N), the degree vector.
!
!    Input/output, integer ( kind = 4 ) QSIZE, the size of indistinguishable
!    supernodes.
!
!    Input/output, integer ( kind = 4 ) QLINK(N), linked list for 
!    indistinguishable nodes.
!
...
!    For each row, the column indices of the nonzero entries.
!
!    Input/output, integer ( kind = 4 ) MASK(N), a mask for the nodes.  Only 
!    those nodes with nonzero input mask values are considered by the routine.
...
!    hold the degree of the nodes in the section graph specified by MASK 
!    and ROOT.
!
  implicit none
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, integer ( kind = 4 ) MASK(N), a mask vector used to specify nodes 
!    in the subgraph.
!
!  Local parameters:
!
!    Local, integer ( kind = 4 ) XLS(N+1), index to a level structure.  The 
!    level structure is stored in part of PERM.
!
  implicit none
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ROOT, the given node, which is not in the 
!    subset S.
!
!    Input, integer ( kind = 4 ) ADJ_ROW(N+1).  Information about row I is 
!    stored in entries ADJ_ROW(I) through ADJ_ROW(I+1)-1 of ADJ.
...
!
!    Input/output, integer ( kind = 4 ) MARKER(N), the marker vector used to 
!    define the subgraph.  Nodes in the subgraph have marker value 0.
...
!    For each row, the column indices of the nonzero entries.
!
!    Input, integer ( kind = 4 ) MASK(N), specifies a section subgraph.  Nodes
!    for which MASK is zero are ignored by ROOT_FIND.
!
!    Output, integer ( kind = 4 ) NLVL, is the number of levels in the level 
...
!    Input, integer ( kind = 4 ) PERM_INV(N), the inverse permutation vector.
!
!    Input/output, integer ( kind = 4 ) MAXSUB, the size of the subscript array 
!    NZSUB.  On return, the actual number of subscripts used.
!
!    Output, integer ( kind = 4 ) IXLNZ(N+1), index into the nonzero storage 
!    vector LNZ.
!
!    Output, integer ( kind = 4 ) XNZSUB(N+1), NZSUB(*), the compressed 
!    subscript vectors.
!
...
!
!    Workspace, integer ( kind = 4 ) RCHLNK(N), used to accumulate the 
!    structure of each column L(*,K).  At the end of the K-th step,
...
!    Input, integer ( kind = 4 ) NBLKS, XBLK(NBLKS+1), the partitioning.
!
!    Input, integer ( kind = 4 ) XENV(N+1), real ( kind = 8 ) ENV(*), the 
!    envelope of the diagonal blocks.
!
!    Input, integer ( kind = 4 ) XNONZ(N+1), real ( kind = 8 ) NONZ(*),
!    integer ( kind = 4 ) NZSUBS(*),
